...
from common.move_classification import HIGH_CRIT_MOVES, PHYSICAL_MOVE_TYPES, SPECIAL_MOVE_TYPES
from common.abilities import (
    ATTACK_DOUBLING_ABILITIES,
    ELECTRIC_IMMUNE_ABILITIES,
    NO_CRIT_ABILITIES,
    THICK_FAT_WEAK_TYPES,
    TRUANT_SPECIES,
    WONDER_GUARD_SUPER_EFFECTIVE_TYPES,
)
...
# Text normalization is centralized in common.text_utils.
_norm = normalize_text

_EXCLUDED_PP_POKEMON: frozenset = frozenset({'mew'})

_SHEDINJA_POKEMON: str = 'shedinja'
    ...
    try:
        ...
        for c in text_cols:
            try:
                ...
                logger.warning(f"Failed to normalize column '{c}': {str(e)}")
                logger.debug(f"Column type: {df[c].dtype}, Column shape: {df[c].shape}")
                # Continue with other columns instead of failing completely
                continue
        
        return df
    except Exception as e:
        logger.error(f"Critical error in normalize_text_columns: {str(e)}")
        logger.debug(f"DataFrame info - Shape: {df.shape if 'df' in locals() else 'Unknown'}")
        logger.debug(f"DataFrame columns: {list(df.columns) if 'df' in locals() else 'Unknown'}")
        raise
...
def get_move_type_boost_multiplier(gen, stage_enc, move_type, stages_df):
    ...
    gen_config = get_generation_config(gen)

    # Check item_type_boost (generations with type-boosting held items)
    if gen_config.has_item_type_boost and 'item_type_boost' in stages_df.columns:
        try:
            ...
            for stage_row in applicable_stages.itertuples(index=False):
                ...
                if move_type in item_types:
                    multiplier *= ITEM_TYPE_BOOST_MULTIPLIER
                    break
        except Exception:
            # If there's any issue, just skip the boost
            pass
    
    # Check badge_type_boost (generations with type-boosting badges)
    if gen_config.has_badge_type_boost and 'badge_type_boost' in stages_df.columns:
        try:
            applicable_stages = stages_df[
                (stages_df['location_stage'] <= stage_enc) &
            ...
            for stage_row in applicable_stages.itertuples(index=False):
                ...
                if move_type in badge_types:
                    multiplier *= BADGE_TYPE_BOOST_MULTIPLIER
                    break
        except Exception:
            # If there's any issue, just skip the boost
            pass
...
def calculate_critical_hit_rate(gen, move_name, base_speed, is_high_crit):
    ...
    config = get_generation_config(gen)
    if config.speed_based_crit:
        # Gen 1: rate derived from base speed; high-crit moves multiply by 8 (capped).
        base_rate = math.floor(base_speed / 2) / 256
        crit_rate = min(8 * base_rate, 255 / 256) if is_high_crit else base_rate
    else:
        # Later generations use fixed normal/high-crit rates.
        crit_rate = config.crit_high if is_high_crit else config.crit_normal
...
def apply_ability_stat_modifiers(gen, pokemon_stats, ability, is_attacker=True):
    ...
    """
    if not get_generation_config(gen).has_abilities or pd.isna(ability):
...
def apply_ability_damage_modifiers(gen, damage, move_type, attacker_ability, defender_ability):
    ...
    """
    if not get_generation_config(gen).has_abilities:
        return damage
    ...
    if defender_ability == 'wonder_guard' and move_type not in WONDER_GUARD_SUPER_EFFECTIVE_TYPES:
        return 0
    elif defender_ability == 'levitate' and move_type == 'ground':
        return 0
    elif defender_ability in ELECTRIC_IMMUNE_ABILITIES and move_type == 'electric':
        return 0
    ...
    if defender_ability == 'thick_fat' and move_type in THICK_FAT_WEAK_TYPES:
        damage *= 0.5
...
def apply_ability_accuracy_modifiers(gen, accuracy, attacker_ability, move_name):
    ...
    """
    if not get_generation_config(gen).has_abilities or pd.isna(attacker_ability):
        return accuracy
...
def apply_ability_crit_modifiers(gen, crit_rate, defender_ability):
    ...
    """
    if not get_generation_config(gen).has_abilities or pd.isna(defender_ability):
        return crit_rate
    
    defender_ability = str(defender_ability).lower()
    
    if defender_ability in NO_CRIT_ABILITIES:
...
def calculate_pokemon_stats_cached(base_stats_tuple, level, generation, stage_enc=None, stages_tuple=None):
    ...
    stat_formula = get_generation_config(generation).stat_formula
    ...
    for stat_name in stat_names:
        ...
        if stat_formula == "gen12":
            ...
            raise ValueError(f"Unsupported stat formula: {stat_formula}")
...
def _is_trade_evolution(gen, pokemon_name, evo_item):
    """Return True if evolving into ``pokemon_name`` requires trading.

    Gen 1 has no hold items, so trade evolutions are identified by species
    name; later generations identify them via the evolution item (the literal
    ``"trade"`` or a specific trade-triggering held item). The rules come from
    the generation registry, so adding a generation only requires a registry
    entry rather than editing this logic.
    """
    config = get_generation_config(gen)
    if config.trade_evolution_mode == "by_name":
        return _norm(pokemon_name) in config.trade_evolution_names
    if pd.notna(evo_item):
        evo_item_norm = _norm(evo_item)
        return evo_item_norm == "trade" or evo_item_norm in config.trade_item_evolutions
    return False
...
def prepare_enc_damage_context(
    *,
    ...
    gl_gen = int(gen)
    gl_has_abilities = get_generation_config(gl_gen).has_abilities
...
def prepare_damage_context(  # <-- same name: drop-in replacement
    *,
    ...
    def _row_calc(pokemon: str, level: int, stage_enc: int, pokemon_enc: str,
        ...
        if gl_has_abilities and ability_enc and str(ability_enc).lower() == "intimidate":
...
{{ ... }}
    ...
    """
    if TQDM_AVAILABLE:
        logger.info(f"Calculating player_pokemons for Generation {gen}")
        logger.info(f"Processing {len(player_pokemon_encounter_pairs_df)} player_pokemon-encounter pairs...")
        logger.info("Loading and normalizing data files...")
        logger.debug("USING UPDATED CODE WITH ZERO-POWER MOVE FIX")
    ...
    try:
        ...
        logger.error(f"Error normalizing input data: {str(e)}")
    ...
    struggle_move = pd.DataFrame({k: [v] for k, v in _STRUGGLE_MOVE.items()})
    ...
    try:
        ...
        logger.error(f"Error normalizing stats data: {str(e)}")
        raise
    
    try:
        stages = normalize_text_columns(stages_raw)
    except Exception as e:
        logger.error(f"Error normalizing stages data: {str(e)}")
        raise
    
    try:
        moves = normalize_text_columns(moves)
    except Exception as e:
        logger.error(f"Error normalizing moves data: {str(e)}")
        raise
    
    try:
        learnsets = normalize_text_columns(learnsets)
    except Exception as e:
        logger.error(f"Error normalizing learnsets data: {str(e)}")
        raise
    
    try:
        exp_types = normalize_text_columns(exp_types)
    except Exception as e:
        logger.error(f"Error normalizing exp_types data: {str(e)}")
        raise
    ...
    if config and config.get('trade_evolutions', 'y').lower() == 'n':
        ...
        if verbose:
            logger.info("Trade evolutions DISABLED - will block evolution into trade Pokemon")
    ...
    def _identify_pokemon(row):
        ...
        for _, pokemon_row in evo_stats.iterrows():
            ...
            is_trade_evolution = _is_trade_evolution(gen, pokemon_name, evo_item)
    ...
    if TQDM_AVAILABLE:
        logger.info("Identifying Pokemon and calculating levels (vectorized)...")
    
    # Create masks for valid data
    if TQDM_AVAILABLE:
        logger.debug("Creating data validity masks...")
    ...
    if valid_mask.sum() == 0:
        if TQDM_AVAILABLE:
            logger.warning("No valid data found, skipping pokemon identification")
    else:
        if TQDM_AVAILABLE:
            logger.debug(f"Processing {valid_mask.sum()} valid player_pokemon-encounter pairs...")
        ...
        if TQDM_AVAILABLE:
            logger.debug("Mapping evo_id to exp_type...")
        ...
        if TQDM_AVAILABLE:
            logger.debug("Calculating Pokemon levels (fully vectorized)...")
    ...
    if TQDM_AVAILABLE:
        logger.info("Filtering duplicate evo_id_pp entries by evolution requirements...")
    ...
    if TQDM_AVAILABLE:
        ...
        logger.debug(f"Removed {removed_count} rows that didn't meet evolution requirements")
        logger.debug("Removed duplicate evo_id_pp entries, keeping highest evo_index_pp")
        logger.debug(f"Filtered dataset: {filtered_count} rows remaining")
    ...
    if 'pokemon' not in combo.columns:
        if TQDM_AVAILABLE:
            logger.warning("Pokemon column missing, reinitializing...")
        combo['pokemon'] = pd.NA
    ...
    # Move-type classification sets (loaded once at import time via common.move_classification)
    physical_moves = PHYSICAL_MOVE_TYPES
    special_moves = SPECIAL_MOVE_TYPES
    crit_moves = HIGH_CRIT_MOVES
    ...
    try:
        ...
        if power_adj_path.exists():
            ...
            if "Name" in power_adj_df.columns and "new_power" in power_adj_df.columns:
                ...
                if verbose:
                    logger.debug(f"Loaded {len(move_power_lookup)} move power adjustments")
                    zero_power_moves = power_adj_df[power_adj_df["new_power"] == 0]["Name"].tolist()
                    if zero_power_moves:
                        logger.debug(f"Zero-power moves to filter: {zero_power_moves}")
    ...
    if 'move' in moves.columns:  
        ...
        if verbose:
            logger.debug(f"Filtered out {initial_move_count - filtered_count} zero-power move entries from moves")
    
    if verbose:
        logger.debug(f"Data loaded successfully. Moves: {len(moves)}, Pokemon: {len(stats)}")
        logger.debug("Setting up optimized lookups for damage calculation...")
    ...
    if 'move_name' in learnsets.columns and 'move' in moves.columns:
        ...
        if verbose and filtered_learnset_count > 0:
            logger.debug(f"Removed {filtered_learnset_count} zero-power move entries from learnsets")
    ...
    if TQDM_AVAILABLE:
        logger.debug("Building optimized lookups for damage calculation...")
    ...
    if TQDM_AVAILABLE:
        logger.debug("Updating base stats columns to match evolved Pokemon...")
    ...
    def _get_pokemon_stats_and_types(pokemon_name, level, gen, stage_enc=None):
        ...
        if _norm(pokemon_name) == _SHEDINJA_POKEMON:
            pokemon_stats['hp'] = 1
    ...
    def _calculate_move_damage(move_name, attacker_stats, defender_stats, attacker_types_set, defender_types, 
        ...
        if gen_config.has_abilities:
            ...
            # If ability is not available or empty, determine from pokemon name (Slakoth and Slaking have Truant)
                ...
                has_truant = pokemon_name_norm in TRUANT_SPECIES
    ...
    if TQDM_AVAILABLE:
        logger.debug("Initializing fast damage computation engines...")
    ...
    if TQDM_AVAILABLE:
        logger.debug(f"Found {len(available_df)} available and {len(unavailable_df)} unavailable player_pokemons")
    
    # Apply function only to available player_pokemons using the fast damage engine
    if not available_df.empty:
        if TQDM_AVAILABLE:
            logger.info(f"Calculating player_pokemon damage for {len(available_df)} available player_pokemons (FAST ENGINE)...")
    ...
    if TQDM_AVAILABLE:
        logger.info(f"Calculating encounter damage for {len(combo)} player_pokemons (FAST ENGINE)...")
    ...
    if 'pokemon_pp' in combo.columns:
        ...
        if TQDM_AVAILABLE:
            logger.debug("Removed pokemon_pp column to avoid confusion (pokemon column is authoritative)")
    
    if TQDM_AVAILABLE:
        logger.info(f"Generation {gen} processing complete!")
        logger.debug(f"Final dataset: {len(combo)} rows with damage calculations")
        logger.debug("Performance: Vectorized operations + pre-computed lookups + categorical optimization")
...
if __name__ == "__main__":
    ...
    logger.debug(f"Processing generations from config: {generations_to_process}")
    ...
    for g in generations_to_process:
        ...
        try:
            ...
            try:
                ...
                logger.info(f"Successfully loaded and normalized data for generation {g}")
            except Exception as e:
                logger.error(f"Error in normalize_text_columns for generation {g}: {str(e)}")
                logger.error(f"Error type: {type(e).__name__}")
                import traceback
                traceback.print_exc()
                continue
            ...
            try:
                ...
                logger.info(f"Successfully calculated player pokemons for generation {g}")
                logger.debug(f"Config settings applied: trade_evolutions={config.get('trade_evolutions', 'y')}")
            except Exception as e:
                logger.error(f"Error in calculate_player_pokemons for generation {g}: {str(e)}")
                logger.error(f"Error type: {type(e).__name__}")
                import traceback
                traceback.print_exc()
                continue
            ...
            logger.debug(f"Gen {g} output saved to {output_path}")
            
        except FileNotFoundError:
            logger.error(f"Missing input file for gen {g}")
            logger.error(f"Expected file: {input_path}")
            logger.error("Please run generate_player_pokemons_x_encounters.py first to create the required input file.")
            break
        except Exception as e:
            logger.error(f"Unexpected error processing generation {g}: {str(e)}")
            logger.error(f"Error type: {type(e).__name__}")
            import traceback
            traceback.print_exc()
            continue
