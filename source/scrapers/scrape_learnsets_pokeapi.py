...
from common.generations import get_generation_config

_POKEAPI_BASE_URL = "https://pokeapi.co/api/v2"

# Generation to version group mapping for PokeAPI
_GENERATION_VERSION_GROUPS = {
    1: ['red-blue', 'yellow'],
...
_STANDARD_LEARN_METHODS = {'level-up', 'machine', 'tutor'}

def load_move_tutor_adjustments() -> pd.DataFrame:
    """Load move tutor adjustments from CSV file."""
    adjustments_path = paths.GEN_ALL_DIR / 'move_tutor_adjustments.csv'
...
def get_pokemon_list_for_generation(generation: int) -> List[int]:
    ...
    The national-dex range (1..national_dex_max) comes from the generation
    registry, so supporting a new generation only requires a registry entry.
    """
    national_dex_max = get_generation_config(generation).national_dex_max
    return list(range(1, national_dex_max + 1))

def fetch_pokemon_data(pokemon_id: int, max_retries: int = 3) -> Optional[Dict]:
    """Fetch Pokemon data from PokeAPI with retry logic."""
    url = f"{_POKEAPI_BASE_URL}/pokemon/{pokemon_id}"
...
def extract_moves_for_generation(pokemon_data: Dict, generation: int, tutor_adjustments: pd.DataFrame = None) -> List[Dict]:
    ...
    moves_data = []
    version_groups = _GENERATION_VERSION_GROUPS[generation]
    ...
    for move_entry in pokemon_data['moves']:
        ...
        for version_detail in move_entry['version_group_details']:
            ...
            if version_group in version_groups:
                ...
                if learn_method in _EXCLUDED_LEARN_METHODS:
                ...
                if learn_method == "tutor" or learn_method not in _STANDARD_LEARN_METHODS:
...
def test_specific_pokemon(generation: int, pokemon_names: List[str]) -> pd.DataFrame:
    ...
    for pokemon_name in pokemon_names:
        ...
        try:
            ...
            url = f"{_POKEAPI_BASE_URL}/pokemon/{api_name}"
            response = requests.get(url, timeout=10)
            response.raise_for_status()
...
if __name__ == "__main__":
    ...
    for move_name in expected_moves:
        ...
        if not found.empty:
            ...
            print(f"  [OK] {move_name} at level {level}")
        else:
            print(f"  [MISSING] {move_name} - NOT FOUND!")
            all_found = False
    ...
    if all_found:
        ...
        for gen in [1, 2, 3]:
            ...
            output_path = str(paths.gen_data_dir(gen) / f"learnsets_gen_{gen}.csv")
