...
def _violates_restrictions(individual: List[int], restriction_groups: List[List[int]]) -> bool:
    """Return True if the selected row indices include 2+ members of any restriction group.

    Used as a defensive post-solve sanity check; the MILP constraints already
    enforce at most one selection per restriction group.
    """
    for group in restriction_groups:
        group_set = set(group)
        if sum(1 for idx in individual if idx in group_set) >= 2:
            return True
    return False


def load_battle_results(path: str) -> pd.DataFrame:
    """Load a battle-results pivot table CSV.

    The expected schema is an ``evo_id_pp`` column plus one column per encounter.
    Handles CSVs where ``evo_id_pp`` was written as the index.
    """
    df = pd.read_csv(path)
...
def calculate_best_party_milp(
    ...
    **kwargs
    ...
    if battle_results_df is not None:
        ...
        df = load_battle_results(battle_results_path)
    else:
    ...
    if restriction_groups and _violates_restrictions(best_party_indices, restriction_groups):
        logger.warning("Selected party violates a restriction group; check restriction configuration.")
...
def save_results_to_file(results: Dict[str, Any], config_path: str = None, df: pd.DataFrame = None, player_pokemon_df: pd.DataFrame = None, party_size: int = None):
    ...
    try:
        ...
        for gen in generations_to_try:
            stats_path = paths.gen_data_dir(gen) / f'stats_gen_{gen}.csv'
            try:
...
if __name__ == "__main__":
    ...
    battle_results_path = str(paths.INTERMEDIATE_DIR / "battle_results_gen1.csv")
