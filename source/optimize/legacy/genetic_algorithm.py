# Deprecated: genetic-algorithm party optimizer, kept for reference only.
# The active optimizer used by the pipeline is optimize/milp.py (MILP).
"""
...
def estimate_best_party(battle_results_path: str, config_path: str = None, thorough: bool = True, **kwargs) -> Dict[str, Any]:
    ...
    restrictions = None
    if config_path:
        restrictions = load_config(config_path).get('restrictions', {})
...
def _save_results_to_file(results: Dict[str, Any], config_path: str = None, optimizer: PokemonPartyOptimizer = None):
    try:
        ...
        config_settings = load_config(config_path) if config_path else {}
