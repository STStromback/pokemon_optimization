...
>>>>>>> 36bb595 (many changes)
    ...
    # variants_df = variants_df[variants_df.evo_id.isin([27])]
    # encounters_df = encounters_df[encounters_df.enc_id.isin([1,800])]
    ...
    # Add evo_index column to indicate evolution sequence within each evo_id
    stats_df['evo_index'] = stats_df.groupby('evo_id').cumcount() + 1

    # Log the columns to verify renaming and evo_index addition
    logging.info(f"Columns in stats_df after renaming and adding evo_index: {stats_df.columns}")
...
def determine_pokemon_by_level(stats_df, merged_df):
    ...
    # Set min_level to the current evo_lvl for each evolution stage
    stats_df_sorted['min_level'] = stats_df_sorted['evo_lvl'].fillna(0)
    # Set max_level to the next evo_lvl for each evolution stage within the same evo_id group
    stats_df_sorted['max_level'] = stats_df_sorted.groupby('evo_id')['evo_lvl'].shift(-1)
    # Fill the max_level of the final evolution stage with infinity
    stats_df_sorted['max_level'] = stats_df_sorted['max_level'].fillna(np.inf)
    # print(stats_df_sorted[['pokemon','evo_lvl','min_level','max_level']].to_string())
    ...
    # print(merged_df['player_level'] >= merged_df['min_level'],merged_df['player_level'] < merged_df['max_level'],merged_df['stage_enc'] >= merged_df['evo_stage'])
    condition = (
    ...
    merged_df = merged_df.sort_values(['enc_id','evo_index'], ascending=[True,True]).groupby(['variant_id', 'enc_id']).tail(1)
    # print(merged_df.to_string())
