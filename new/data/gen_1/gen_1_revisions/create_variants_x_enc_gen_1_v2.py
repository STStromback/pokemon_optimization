...
def read_csv_files():
    ...
    # DEBUG
    # variants_df = variants_df[variants_df.variant_id == 304]
...
def determine_pokemon_by_level(stats_df, merged_df):
    ...
    logging.info("Pokémon determined by level successfully.")
    return merged_df
...
def assign_best_moves(row, moves_df):
    ...
    for move_type in variant_types:
        ...
        if not valid_moves.empty:
            ...
            valid_moves.loc[:, 'expected_damage'] = valid_moves.apply(lambda m: calculate_expected_damage(m, base_speed), axis=1)
