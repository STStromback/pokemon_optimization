...
def read_csv_files():
    ...
    # DEBUG
    # variants_df = variants_df[variants_df.evo_id == 1]
    ...
    logging.info(f"Columns in stats_df after renaming: {stats_df.columns.tolist()}")
...
def merge_and_calculate_levels(cross_join_df, stats_df, exp_table_melted):
    ...
    merged_df = pd.merge(cross_join_df, stats_df[[
        'evo_id', 'exp_type', 'base_hp', 'base_attack', 'base_defense', 'base_sp_attack', 'base_sp_defense',
        'base_speed']], on='evo_id')
...
def calculate_stats(base_stats, level):
    ...
    # Ensure that all calculations return integers
...
def determine_pokemon_by_level(stats_df, merged_df):
    ...
    # Log the columns to verify the merge
    logging.info(f"Columns in merged_df after merging with stats_df: {merged_df.columns.tolist()}")
    ...
    merged_df = merged_df.rename(columns={
        'base_hp_x': 'base_hp',
        'base_attack_x': 'base_attack',
        'base_defense_x': 'base_defense',
        'base_sp_attack_x': 'base_sp_attack',
        'base_sp_defense_x': 'base_sp_defense',
    ...
    for index, row in merged_df.iterrows():
        ...
        merged_df.at[index, 'hp'] = int(calculated_stats['hp'])
        merged_df.at[index, 'attack'] = int(calculated_stats['attack'])
        merged_df.at[index, 'defense'] = int(calculated_stats['defense'])
        merged_df.at[index, 'sp_attack'] = int(calculated_stats['sp_attack'])
        merged_df.at[index, 'sp_defense'] = int(calculated_stats['sp_defense'])
        merged_df.at[index, 'speed'] = int(calculated_stats['speed'])
