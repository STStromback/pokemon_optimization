...
def main(file_path='../gen_1_data_curated/ehl_pivot.csv', stats_file='../gen_1_data_raw/stats_gen_1.csv', variants_file='../gen_1_data_curated/variants_gen_1.csv',
    ...
    best_variant_ids_df.to_csv('../gen_1_output/best_variant_ids.csv', index=False)

    # Get final evolution names and move types
    final_evolutions = get_final_evolution_names(stats_file, variants_file)

    # Merge the selected rows with final evolution names and move types
    result = selected_rows.merge(final_evolutions, on='variant_id', how='left')

    # Prepare output text
    output_lines = ["Team Composition:"]
    for _, row in result.iterrows():
        types = ', '.join([str(row[t]) for t in ['move_type_1', 'move_type_2', 'move_type_3', 'move_type_4'] if pd.notna(row[t]) and row[t] != ''])
...
def plot_results(logbook):
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig("../gen_1_output/fitness_over_generations.png")
    plt.show()
...
def generate_additional_outputs(filtered_df, best_variant_ids, stats_file, variants_file):
    ...
    sorted_matrix_df.to_csv('../gen_1_output/sorted_matrix1.csv', index=False)
    ...
    matrix = pd.read_csv('../gen_1_output/sorted_matrix1.csv')
    ...
    plt.savefig('../gen_1_output/pokemon_encounter_heatmap_normalized_with_min_gen1.png')
    plt.show()
    ...
    plt.savefig('../gen_1_output/pokemon_unique_min_encounter_heatmap_gen1.png')
    plt.show()
    ...
    plt.savefig('../gen_1_output/pokemon_all_min_encounter_heatmap_gen1.png')
    plt.show()
    ...
    worst_encounters_df.to_csv('../gen_1_output/worst_encounters_gen1.csv', index=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--file_path', type=str, default='../gen_1_data_curated/ehl_pivot.csv', help='Path to the input CSV file.')
    parser.add_argument('--stats_file', type=str, default='../gen_1_data_raw/stats_gen_1.csv',
                        help='Path to the stats CSV file.')
    parser.add_argument('--variants_file', type=str, default='../gen_1_data_curated/variants_gen_1.csv',
