...
def main():
    # Load data
    moves_data = pd.read_csv('../gen_1_data_raw/moves_gen_1.csv')
    moves_data = moves_data.dropna(subset=['move_type'])  # Ensure all entries have a type

    # Load mapping
    stage_map = pd.read_csv('../gen_1_data_raw/stages_gen_1.csv')
    ...
    # Save to CSV
    variants_df.to_csv('../gen_1_data_curated/variants_gen_1.csv', index=False)
