...
def calc_se(df, gen):
    ...
    def get_boosts(original, move_type, enc_stage, pokemon):
        ...
        if pokemon in ['Cubone', 'Marowak'] and move_type in ['Normal', 'Fighting', 'Flying', 'Poison', 'Ground', 'Rock', 'Bug', 'Ghost', 'Steel']:
            original *= 2
    ...
    if gen == 2:
        for i in range(1, 5):
            df[f'move{i}_se'] = df.apply(lambda row: get_boosts(row[f'move{i}_se'], row[f'move{i}_type'], row['stage_enc'], row['pokemon']), axis=1)
